  ...
  try {
    ...
    for (const field of fields) {
      if (field === 'price') {
        ...
        if (error) {
          console.error('[GET FILTERS API ERROR] Error fetching max price:', {
        ...
        if (error) {
          console.error(`[GET FILTERS API ERROR] Error fetching ${field}:`, {
            message: error.message,
            timestamp: new Date().toISOString(),
          });
          continue;
    ...
    console.log('[GET FILTERS API] Filters completed', {
    ...
    console.error('[GET FILTERS API] Unexpected error:', error);
